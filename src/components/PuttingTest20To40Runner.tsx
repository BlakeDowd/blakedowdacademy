...
export type Putting20To40HoleRecord = {
  holeIndex: number;
  distance: number;
  shape: ShapeKey;
  outcome: "make" | "miss";
...
const HOLES = puttingTest20To40Config.holeCount;
...
function buildHoles20To40(): HoleSetup[] {
  const distances = shuffle([...puttingTest20To40Config.distances]);
  const shapePool: ShapeKey[] = [
    ...Array(puttingTest20To40Config.straightCount).fill("Straight" as const),
    ...Array(puttingTest20To40Config.leftToRightCount).fill("Left-to-Right" as const),
    ...Array(puttingTest20To40Config.rightToLeftCount).fill("Right-to-Left" as const),
  ];
...
async function persistHoleToSupabase(userId: string, record: Putting20To40HoleRecord) {
  try {
    const { createClient } = await import("@/lib/supabase/client");
    const supabase = createClient();
    const { error } = await supabase.from("practice").insert({
      user_id: userId,
      type: puttingTest20To40Config.practiceType,
      duration_minutes: 0,
      notes: JSON.stringify({
        kind: puttingTest20To40Config.noteKind,
    ...
    if (error) {
      console.warn("[PuttingTest20To40] Supabase save:", error.message);
    } else {
      if (record.holeIndex === puttingTest20To40Config.holeCount - 1) {
    ...
    console.warn("[PuttingTest20To40] Supabase save failed", e);
  }
}
...
export function PuttingTest20To40Runner() {
  ...
  const [holeLog, setHoleLog] = useState<Putting20To40HoleRecord[]>([]);
  ...
  const finishHole = useCallback(
    (record: Putting20To40HoleRecord) => {
      flushSync(() => {
    ...
    return (
      <div className="mt-6 space-y-4">
        ...
        <p className="text-sm text-gray-600">
          Ten lag putts from 20–40 feet. Distances are shuffled each run with two straight, four
    ...
    return (
      <div className="mt-6 space-y-5">
        <div className="space-y-1">
          <p className="text-sm font-semibold text-gray-900">{puttingTest20To40Config.testName}</p>
          <h2 className="text-base font-medium text-gray-600">Test complete</h2>
        </div>
        <PuttingTestResultsSummaryCard points={summary.points} putts={summary.putts} variant="20to40" />
    ...
    <div className="mt-6 space-y-6">
      ...
      {phase === "second-putt" && missCategory != null && primaryMissReason != null && (
        <div className="space-y-4">
          <div>
            <label htmlFor="second-putt-dist-2040" className="block text-sm font-medium text-gray-700 mb-1">
              Second putt distance (ft)
            </label>
            <input
              id="second-putt-dist-2040"
              type="text"
              inputMode="decimal"
              placeholder="e.g. 3"
