...
const HOLES = puttingTest9Config.holeCount;
...
function buildHoles9(): HoleSetup[] {
  const distances = shuffle([...puttingTest9Config.distances]);
  const shapePool: ShapeKey[] = [
    ...Array(3).fill("Straight" as const),
    ...Array(3).fill("Left-to-Right" as const),
    ...Array(3).fill("Right-to-Left" as const),
  ];
...
async function persistHoleToSupabase(userId: string, record: Putting9HoleRecord) {
  try {
    const { createClient } = await import("@/lib/supabase/client");
    const supabase = createClient();
    const { error } = await supabase.from("practice").insert({
      user_id: userId,
      type: puttingTest9Config.practiceType,
      duration_minutes: 0,
      notes: JSON.stringify({
        kind: puttingTest9Config.noteKind,
...
export function PuttingTest9Runner() {
  ...
  const [holeLog, setHoleLog] = useState<Putting9HoleRecord[]>([]);
    ...
    return (
      <div className="mt-6 space-y-4">
        ...
        <p className="text-sm text-gray-600">
          Nine putts with fixed distances (shuffled each run). Three straight, three left-to-right, and
          three right-to-left breaks assigned at random. Same scoring as the 18-hole test, including miss
          diagnostics.
        </p>
        <button
          type="button"
    ...
    return (
      <div className="mt-6 space-y-5">
        <div className="space-y-1">
          <p className="text-sm font-semibold text-gray-900">{puttingTest9Config.testName}</p>
          <h2 className="text-base font-medium text-gray-600">Test complete</h2>
        </div>
        <PuttingTestResultsSummaryCard points={summary.points} putts={summary.putts} variant={9} />
    ...
    <div className="mt-6 space-y-6">
      ...
      {phase === "second-putt" && missCategory != null && primaryMissReason != null && (
        <div className="space-y-4">
          <div>
            <label htmlFor="second-putt-dist-9" className="block text-sm font-medium text-gray-700 mb-1">
              Second putt distance (ft)
            </label>
            <input
              id="second-putt-dist-9"
              type="text"
              inputMode="decimal"
              placeholder="e.g. 3"
